    ...
    try:
        ...
        if hasattr(sound, 'to_mfcc') and hasattr((mfcc := sound.to_mfcc(number_of_coefficients=13)), 'to_array'):
            ...
            for i in range(min(13, mfcc_matrix.shape[0])):
                ...
                if len(coefs) > 0:
                    # ОСЛЕПЛЕНИЕ: Убираем средние/мин/макс для MFCC-0 и MFCC-1
                    if i in [0, 1]:
                        features.extend([0.0, np.std(coefs), 0.0, 0.0, np.max(coefs)-np.min(coefs), 0.0])
                    else:
        ...
        # 2. Pitch (Высота голоса)
        pitch_vals = call(sound, "To Pitch", 0.0, 75, 500).selected_array['frequency']
        pitch_vals = pitch_vals[pitch_vals > 0]
        # ОСЛЕПЛЕНИЕ: Убираем среднюю, мин, макс и медианную высоту голоса. Оставляем std и размах.
        features.extend([0.0, np.std(pitch_vals), 0.0, 0.0, np.max(pitch_vals)-np.min(pitch_vals), 0.0] if len(pitch_vals) > 0 else [0.0]*6)
        
        # 3. Intensity (Громкость - не зависит от пола)
        ...
        for f_num in [1, 2, 3]:
            ...
            # ОСЛЕПЛЕНИЕ: Убираем средние частоты формант, оставляем только их изменчивость (std)
            features.extend([0.0, np.std(f_vals), 0.0, 0.0] if f_vals else [0.0]*4)
