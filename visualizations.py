...
def plot_comprehensive_cms(metrics, title_prefix, filename):
    ...
    # Row 1: Overall (Centered, others empty)
    ...
    # Row 3: Language
    for i in range(3): draw_cm(axes[2, i], metrics['cm_lang'].get(i), LANG_NAMES[i])
...
def generate_text_report(fold_results, filename="extended_report.txt"):
    ...
    with open(filepath, 'w', encoding='utf-8') as f:
        f.write("="*50 + "\nРАСШИРЕННЫЙ ОТЧЕТ О КРОСС-ВАЛИДАЦИИ\n" + "="*50 + "\n\n")
        ...
        f.write(f"СРЕДНИЕ МЕТРИКИ (ПО ВСЕМ 5 ФОЛДАМ):\n")
        ...
        for r in fold_results:
            ...
            # 🆕 Детальный вывод метрик для каждого фолда
            ...
            for g, cm in r['cm_gender'].items(): 
                f.write(f"{GENDER_NAMES[g]}:\n{cm.tolist()}\n")
            f.write("\n")